...
{% docs event_type %}
Used in reporting. The hardcoded higher-level group for the `event_category` value, generated by the macro `assign_event_type`. Examples include: Error, Navigation.
{% enddocs %}
...
Count of distinct `resource_id` per program and resource type combination.
Used to filter out one-off cases so the programs with type 'activity' available are accurate.
{% enddocs %}
...
{% docs path_entered %}
For 'router.left' events, the path the user navigated to.
{% enddocs %}


{% docs path_left %}
For 'router.left' events, the path the user navigated away from.
{% enddocs %}
