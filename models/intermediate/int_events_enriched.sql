...
event_metadata as (
    select * exclude (proposed_new_event_name)
    from
...
joined as (
    ...
    left join
        ...
        on
            ...
            -- but relationship test is in the events staging model
...
events_add_column_info as (
    select
        joined.*,
        date(joined.server_timestamp) as server_event_date,
        date(joined.client_timestamp) as client_event_date,
        -- events with unique behaviors
        case
            -- router.left events: 
            -- path = route user navigating TO
            -- event_value = route user just LEFT
            when joined.event_name = 'router.left' then joined.event_path
            when joined.event_name = 'router.enter' then joined.event_value
        end as path_entered,
        case
            when joined.event_name = 'router.left' then joined.event_value
        end as path_left,

        -- joins to the focuses_areas.value (source name) text
        case
            when
                joined.event_name in (
                    'weekly.planner.program.week.filter.deselect',
                    'weekly.planner.program.week.filter.select'
                )
                then joined.event_value_human_readable
        end as focus_area_label,

        -- extract info such as week_number from planner paths
        -- TAG IMPROVEMENT
        -- extract theme vs month_number: join to resource_id when not a month 
        case
            when joined.event_name in {{ planner_path_events }}
                then try_cast(
                    regexp_substr(
                        joined.event_path,
                        '^/planner/[^/]+/[^/]+/([0-9]+)$',
        ...
        try_cast(
            coalesce(
                case
                    when
                        joined.event_value_joins_to = 'program_id'
                        and try_cast(joined.event_value as integer)
                        then joined.event_value
                end,
                regexp_substr(
                    joined.event_path, 'resources/([^/]+)', 1, 1, 'e', 1
                ),
                regexp_substr(
                    joined.event_path, 'planner/([^/]+)', 1, 1, 'e', 1
                ),
                regexp_substr(
                    joined.event_value, 'resources/([^/]+)', 1, 1, 'e', 1
                )
            ) as integer
        ) as program_id,
        ...
        try_cast(
            coalesce(
                case
                    when
                        joined.event_value_joins_to = 'resource_id'
                        and try_cast(joined.event_value as integer)
                        then joined.event_value
                end,
                regexp_substr(
                    joined.event_path, 'detail/([0-9]+)', 1, 1, 'e', 1
                ),
                regexp_substr(
                    joined.event_value, 'detail/([0-9]+)', 1, 1, 'e', 1
                )
            ) as integer
        ) as resource_id
        ...
        {% if id_join_columns -%}
            {%- for col in id_join_columns -%}
                {%- if col and col not in manually_added_columns -%}
                    ...
                    case
                        when
                            events_add_column_info.event_value_joins_to
                            = '{{ col }}'
                            then
                                try_cast(
        ...
        {% if not_id_columns -%}
            {%- for col in not_id_columns -%}
                {%- if col and col not in manually_added_columns -%}
                    ...
                    case
                        when
                            events_add_column_info.event_value_not_id
                            = '{{ col }}'
                            then events_add_column_info.event_value
                    end as {{ col }}
                {%- endif -%}
            {%- endfor -%}
        {%- endif -%}
