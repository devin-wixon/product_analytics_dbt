...
user_categories as (
    select
        ...
        case
            -- backfill users all have dbt_valid_from = 1900-01-01 (unknown entry date)
...
user_ever_not_invited as (
    select
        ...
        min(
            case when user_invite_status = 'not_invited' then dbt_valid_from end
        ) as min_user_not_invited_date_to_check,
        -- null out the date when we don't have a real date
        case
            when
                min_user_not_invited_date_to_check::date = '1900-01-01'
                then null
            else min_user_not_invited_date_to_check
        end as min_user_not_invited_date
...
-- if any event but no record of the status, count as status with no date

-- Check if user was ever invited and get min date
-- user_email_sent_at_utc is the most recent invite sent; will often be null (see docs)
        ...
        case
            when
                boolor_agg(
                    ...
                    or user_email_sent_at_utc is not null
                )
                then true
            when boolor_agg(has_user_event) then true
            else false
        end as is_user_invited,
        min(case when user_invite_status = 'invited' then dbt_valid_from end)
            as min_user_invited_date_to_check,
        -- null out the date when we don't have a real date
        coalesce(
            case
                when
                    min_user_invited_date_to_check::date = '1900-01-01'
                    then null
                else min_user_invited_date_to_check
            end,
...
user_ever_registered as (
    select
        ...
        min(
            case when user_invite_status = 'registered' then dbt_valid_from end
        ) as min_user_register_date_to_check,
        -- null out the date when we don't have a real date
        case
            when
                min_user_register_date_to_check::date = '1900-01-01'
                then null
            else min_user_register_date_to_check
        end as min_user_register_date
    from user_history_enriched
    where
        user_category = 'username_password'
    group by user_id
...
user_active_days as (
    select
        user_id,
        min(case when user_day_index = 1 then event_date end)
            as min_user_active_date,
        min(case when user_day_index = 2 then event_date end)
            as second_user_active_date,
...
user_funnel_base as (
    select
        ...
        case
            when
                user_categories.user_category in ('legacy', 'backfill')
                then null
            -- month_start_date will never be null
            else user_first_record_month.month_start_date
        end as month_start_date,
        coalesce(user_active_days.is_user_active, false) as is_user_active,
        coalesce(user_active_days.is_user_active_two_days, false)
            as is_user_active_two_days,
        coalesce(user_active_days.n_user_active_days, 0) as n_user_active_days,
