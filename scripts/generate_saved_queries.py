#!/usr/bin/env python3
"""
Generate saved queries YAML for Tableau parameterization.

This script creates all combinations of time grains and dimensions
for comprehensive Tableau parameterization while maintaining DRY principles.
"""
...
def get_dimensions():
    """Define all available dimensions for Tableau parameterization."""
    return [
...
def generate_saved_query(time_grain, dimension_combo):
    ...
    # Build group_by list
    group_by = [f"TimeDimension('{time_grain['semantic_ref']}', '{time_grain['name']}')"]
...
def main():
    ...
    # Write to file
    output_path = '../models/semantic_models/saved_queries_tableau_generated.yml'
    with open(output_path, 'w') as f:
        f.write("# Generated by scripts/generate_saved_queries.py\n")
        f.write("# Purpose: Dynamically generated saved queries for Tableau parameterization\n")
        f.write(f"# Generates {len(saved_queries)} queries for all time grains × dimension combinations\n\n")
    ...
    sources_path = '../models/staging/query_exports/_sources_tableau_generated.yml'
    with open(sources_path, 'w') as f:
        f.write("# Generated by scripts/generate_saved_queries.py\n\n")
        yaml.dump(sources_output, f, default_flow_style=False, sort_keys=False, width=1000)
